...
@Component({
    ...
    <div class="login-wrapper">
      <div class="login-card glass-panel animate-fade-in">
        <div class="login-header">
          <div class="brand">
            <span class="material-icons brand-icon">apartment</span>
            <h2>DormPortal</h2>
          </div>
          <p class="subtitle">Secure Dormitory Management Hub</p>
        </div>
        ...
        <div class="demo-creds">
          ...
          <div class="creds-grid">
            ...
            <div class="cred-item">
              <span class="cred-role">Dormer:</span>
              <span class="cred-val">dormer1 / dormerpass</span>
            </div>
          </div>
        </div>
      </div>
  ...
  styles: [`
    .login-wrapper {
      ...
      align-items: center;
      min-height: 100vh;
      background: radial-gradient(circle at 50% 50%, #0f1026 0%, #06080d 100%);
      padding: 20px;
    }
    ...
    this.authService.login(username, password).subscribe({
      next: (session) => {
        ...
        if (session.role === 'ROLE_ADMIN') {
          ...
          this.router.navigate(['/dormer-dashboard']);
        }
      },
      error: (err) => {
        this.isLoading = false;
